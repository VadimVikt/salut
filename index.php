...
$catalog ='ff/imp/catalog_test.csv';
$offers = 'ff/imp/offers1_test.csv';
...
foreach ($resOffers['XML_ID'] as $key => $value) {
    ...
    $resCatalog['PRICE'][$key] =  ($resOffers['AB6F91A1-69DA-4132-9556-C2565FCD97B4'][$key]);
    $resCatalog['QUANTITY'][$key] =  intval($resOffers['QUANTITY'][$key]);
}
//dd($resCatalog);
...
for ($i = 0; $i < count($resCatalog['XML_ID']); $i++) {
    ...
    $result[$i]['MIN_ORDER'] = $resCatalog['NAIMENOVANIETRETEGORAZRJADA'][$i];
